...
uv run python -m scripts.check_models                           # ping every model in config.py once (verify a provider switch works)
...
- **Model selection lives in one place: `app/config.py`.** Every feature (Ask, Lesson, Summary, Assessment, Review, Prep, OCR, embeddings) maps to a model there; call sites pass `config.<FEATURE>` to `track_claude` / `track_gemini` / `track_gemini_embed` (never a hardcoded model string). To change a model, edit `config.py` only. `clients._PRICING` is keyed off the same config ids. **Model tiering is deliberate:** `config.CLAUDE_SONNET` is reserved for three quality-sensitive paths only (`ASSESSMENT_GENERATE`, `ASSESSMENT_REGEN_TEXT_ONLY`, `GRADING`); everything else uses `config.CLAUDE_HAIKU` (~1/3 cost). Ingest OCR uses `gemini-2.5-flash-lite` (typed/printed, `config.OCR`) and `gemini-2.5-flash` (handwriting, `config.OCR_HANDWRITING`); embeddings use `gemini-embedding-001` (`config.EMBED`). Cross-provider switching works: a feature can name a Claude, Gemini, or OpenAI model and `track_claude`/`track_gemini`/`track_gemini_embed` route automatically (provider inferred from the model name via `clients.provider_of`). Native Claude/Gemini calls are unchanged; a non-native model id is translated through neutral message turns + a per-provider adapter and the response is shimmed so call sites still read `.content[0].text` / `.text` / `.embeddings`. OpenAI needs `OPENAI_API_KEY`; Claude has no embeddings; changing `config.EMBED` requires re-embedding all chunks at 1536 dims.
